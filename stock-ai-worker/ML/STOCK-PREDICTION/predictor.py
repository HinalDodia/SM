...
#             "macd": round(float(row["MACD"]), 2) if pd.notnull(row.get("MACD")) else None,
#         }
...
#             next_day_std = std_price
#             next_day_samples = rescaled_samples.tolist()
...
#             # Raw MC-Dropout samples for this day (40 values) — lets the frontend
#             # draw a distribution histogram instead of just mean/low/high.
#             "mc_samples": [round(float(s), 2) for s in rescaled_samples],
#         })
...
#     latest_macd = float(data_df["MACD"].iloc[-1]) if "MACD" in data_df else 0.0

#     # ── Trend Probability — % of day-1 MC-Dropout samples above/below current price ──
#     if next_day_samples:
#         samples_arr = np.array(next_day_samples)
#         bullish_pct = round(float(np.mean(samples_arr > current_price)) * 100, 1)
#         bearish_pct = round(float(np.mean(samples_arr < current_price)) * 100, 1)
#     else:
#         bullish_pct = bearish_pct = 0.0

#     # ── Risk Score (0-100, higher = riskier) ──
#     # Blends day-1 volatility (std relative to price) with how wide the
#     # predicted range is relative to the predicted price. Both quantities
#     # were already computed above for the confidence score — this just
#     # reframes them as "risk" instead of discarding them.
#     volatility_component = min(100.0, (next_day_std / current_price) * 100 * 5) if current_price else 0.0
#     range_width_pct = ((next_day_high - next_day_low) / next_day_predicted_price * 100) if next_day_predicted_price else 0.0
#     range_component = min(100.0, range_width_pct * 3)
#     risk_score = round(min(100.0, volatility_component * 0.6 + range_component * 0.4), 1)

#     # ── AI Signal (BUY / HOLD / SELL) ──
#     # Simple, transparent point-scoring from signals already computed above.
#     # Not a separate model — just a combined read of trend + confidence + RSI + MACD.
#     score = 0
#     if trend == "UP":
#         score += 1
#     elif trend == "DOWN":
#         score -= 1
#     if next_day_confidence > 60:
#         score += 1 if trend == "UP" else (-1 if trend == "DOWN" else 0)
#     if latest_rsi < 30:
