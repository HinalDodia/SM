...
NSE equivalent of short interest for F&O stocks.
Combines yfinance price data with NSE short-selling, delivery
position, and price-volume data from nselib.
"""
...
def _fetch_oi_mwpl_pcr(symbol: str) -> dict:
    """
    Collect OI change, MWPL proxy, PCR proxy, delivery %, and trade count
    from NSE via nselib. All fields default to None on failure.
    """
    result = {
        "oi_change_pct":       None,
        "mwpl_pct":            None,
    ...
    try:
        ss_df = capital_market.short_selling_data(from_date=from_str, to_date=to_str)
    ...
    try:
        ...
        if not del_df.empty:
            ...
            delivery_pct               = float(del_df.iloc[0]["%DlyQttoTradedQty"])
            result["delivery_pct"]     = delivery_pct
            ...
            result["mwpl_pct"] = round(100 - delivery_pct, 2)

    except Exception as e:
        print(f"[nselib] deliverable_position_data failed for {symbol}: {e}")

    # 3. Price-volume data → PCR proxy + trade count
    try:
