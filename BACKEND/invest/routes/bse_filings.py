"""
/bse-filings/<symbol>
/bse-filings/<symbol>/download
/bse-company/<symbol>
/companies

All BSE-related routes: listing companies, fetching filings with
category filters, downloading PDFs, and looking up BSE company info.
"""
...
# BSE's exact strCat string for financial results
...
ALLOWED_PDF_DOMAINS = ("bseindia.com", "nseindia.com")
...
CATEGORY_MAP = {
    ...
    "board meeting":               "Board",      "outcome of board":    "Board",
    "board":                       "Board",      "dividend":            "Dividend",
    "date of payment of dividend": "Dividend",   "record date":         "Dividend",
    "agm":                         "AGM",         "egm":                 "AGM",
    "annual general meeting":      "AGM",         "extraordinary general": "AGM",
    "insider":                     "Insider",     "sast":                "Insider",
    "shareholding":                "Insider",     "beneficial ownership": "Insider",
    "trading window":              "Insider",     "pit regulation":      "Insider",
    "acquisition":                 "Acquisition", "merger":              "Acquisition",
    "buyback":                     "Acquisition", "incorporation":       "Acquisition",
    ...
    "reg.24":                      "Compliance",  "secretarial compliance": "Compliance",
    "lodr":                        "Compliance",  "regulation 30":       "Compliance",
    "newspaper publication":       "Compliance",  "newspaper":           "Compliance",
}

# Subcategory substrings that confirm a genuine financial result
_RESULT_SUBCATEGORY_WHITELIST = [
    "financial result", "unaudited result", "audited result",
    "quarterly result", "half yearly result", "half-yearly result",
    "annual result", "standalone result", "consolidated result",
]

# Headline keywords that also confirm a result (fallback)
_CONFIRM_RESULT_KEYWORDS = [
    "financial result", "unaudited result", "audited result",
    "quarterly result", "half yearly result", "half-yearly result",
    "standalone result", "consolidated result",
    "results for the quarter", "results for the year", "results for the half",
]

# Headlines that always disqualify, even inside the Results category
_HARD_EXCLUDE_HEADLINES = [
    "press release", "newspaper", "investor presentation",
    "earnings call", "transcript", "concall", "audio recording",
]
...
# Load once at module startup and attach known BSE codes
...
def detect_quarter(headline: str, date_raw: str) -> str | None:
    """
    Detect Q1/Q2/Q3/Q4 from headline text, fallback to filing month.
...
def get_bse_scrip(symbol: str) -> str | None:
    """Look up BSE scrip code from the static map, or fall back to BSE API."""
...
def bse_filings(symbol):
    ...
    # Do NOT pass display category names (e.g. "Board") as strCat — BSE uses
    # different internal strings. We filter client-side after fetching.
    str_cat = BSE_RESULT_CAT if results_only else "-1"

    # Split the date range into 90-day chunks (BSE API limit per request)
    chunks = []
    chunk_start = from_dt
    ...
    filings.sort(
        key=lambda x: (x["date"] or "", x["description"] or "", x["pdf_url"] or ""),
        reverse=True,
    )
...
def download_filing_pdf(symbol):
    ...
    if attach:
        ...
        if not any(d in pdf_url for d in ALLOWED_PDF_DOMAINS):
    ...
    for url in candidates:
        if not any(d in url for d in ALLOWED_PDF_DOMAINS):
            continue
        try:
